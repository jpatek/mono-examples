...
static gboolean key_press_event (GtkWidget   *widget,
							     GdkEventKey *event,
							     gpointer     user_data);
static void title_changed (GObject *widget,
						   GParamSpec *property,
						   gpointer data);
          ...
          char *argv[])
{
  ...
  // Use g_object_set instead of the gtk_ helper functions
  //gtk_window_set_title (GTK_WINDOW (window), "Thomas");
  g_object_set (window, "title", "Thomas", NULL);
  
  // Notify when the title changes
			   ...
			   gpointer     user_data)
{
	...
	if (g_ascii_strcasecmp(tmp_title, "Thomas") == 0)
	{
		...
		g_object_set (window, "title", "Wiest", NULL);
		
		//gtk_label_set_text (GTK_LABEL (label), "Thomas");
		g_object_set (label, "label", "Thomas", NULL);	
	}
	else
	{
		// Use g_object_set instead of the gtk_ helper functions
		...
		g_object_set (label, "label", "Wiest", NULL);
	}
}
